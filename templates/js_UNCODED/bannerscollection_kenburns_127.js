...
(function($) {
	...
	function animate_texts(current_obj,options,bannerscollection_kenburns_the,bannerControls) {
		...
		texts.each(function() {
		  ...
		  if (options.responsive) {
            ...
            currentText_arr[i].css("opacity",parseInt(currentText_arr[i].attr('data-fade-start'))/100,10); 
	...
	function initialPositioning(cur_i,options, bannerscollection_kenburns_container,origImgsDimensions,imgs) {
		...
		imgInside.css('width',parseInt(cur_initialZoom*origDim[0],10)+'px');
		imgInside.css('height',parseInt(cur_initialZoom*origDim[1],10)+'px');
		...
		imgInside.css('left',parseInt(cur_left,10)+'px');
		imgInside.css('top',parseInt(cur_top,10)+'px');
			...
			origDim[1]=origDim[1]/(options.origWidth/options.width);
				...
				if (jQuery.browser.msie) {
					if (options.width100Proc)
						cur_duration=cur_duration+options.durationIEfix;
					curZoom=1;
					zoomStep=0;
					cur_marginLeft=0;
					cur_marginTop=0;
				    ...
				    current_obj.msiInterval=setInterval(function() {
						...
						if ( (nowx-current_obj.msiInitialTime) > (cur_duration*1000) ) {
							...
							} else if (cur_horizontalPosition=='right') {
							...
							} else if (cur_verticalPosition=='bottom') {
		...
		if (Math.abs(currentCarouselLeft-current_obj.carouselStep)<(thumbsHolder_Thumb.width()+thumbMarginLeft)*total_images) {
			...
			function rearangethumbs(current_obj,options,total_images,bannerscollection_kenburns_container,thumbsHolder_Thumbs,bannerscollection_kenburns_thumbsHolder,bannerscollection_kenburns_carouselLeftNav,bannerscollection_kenburns_carouselRightNav,thumbMarginLeft,thumbsHolder_Thumb,bannerscollection_kenburns_thumbsHolderVisibleWrapper,bannerscollection_kenburns_thumbsHolderWrapper) {
						...
						if (options.skin!="opportune") {
							...
							bannerscollection_kenburns_thumbsHolderWrapper.css("height",parseInt(options.origthumbsHolderWrapperH/(options.origWidth/options.width),10)+'px');
							...
							thumbsHolder_Thumbs.css('width',parseInt(options.origThumbW/(options.origWidthThumbsHolderVisibleWrapper/bannerscollection_kenburns_thumbsHolderVisibleWrapper.width()),10)+'px');
							thumbsHolder_Thumbs.css('height',parseInt(options.origThumbH/(options.origWidthThumbsHolderVisibleWrapper/bannerscollection_kenburns_thumbsHolderVisibleWrapper.width()),10)+'px');
							
							if (options.numberOfThumbsPerScreen >= total_images) {
								bannerscollection_kenburns_thumbsHolderVisibleWrapper.css('left',parseInt((bannerscollection_kenburns_thumbsHolderWrapper.width() - (thumbsHolder_Thumb.width()+thumbMarginLeft)*total_images)/2,10)+'px');
							}							
							...
							img_inside.css("margin-top",parseInt((bannerscollection_kenburns_thumbsHolderWrapper.height()-thumbsHolder_Thumbs.height())/2,10)+"px");
			...
			function doResize(current_obj,options,total_images,imgs,bannerscollection_kenburns_the,bannerControls,bannerscollection_kenburns_contentHolderVisibleWrapper,bannerscollection_kenburns_contentHolder,bannerscollection_kenburns_container,bannerscollection_kenburns_playOver,origImgsDimensions,thumbsHolder_Thumbs,bottomNavButs,bannerscollection_kenburns_thumbsHolder,bannerscollection_kenburns_carouselLeftNav,bannerscollection_kenburns_carouselRightNav,thumbMarginLeft,thumbsHolder_Thumb,bannerscollection_kenburns_leftNav,bottomNavBut,bannerscollection_kenburns_bottomNav,bannerscollection_kenburns_thumbsHolderVisibleWrapper,bannerscollection_kenburns_thumbsHolderWrapper) {
					...
					if (options.origWidth!=responsiveWidth || options.width100Proc) {
						...
						for (i=0; i<total_images; i++) {
							...
							jQuery(jQuery(imgs[i]).attr('data-text-id')).css('left',parseInt(i*options.width,10));
						...
						if (options.skin=="opportune") {
							bannerscollection_kenburns_bottomNav.css("left",parseInt((bannerscollection_kenburns_container.width()-bannerscollection_kenburns_bottomNav.width())/2,10)+'px');
					...
					//playover
					bannerscollection_kenburns_playOver.css('left',parseInt((options.width-bannerscollection_kenburns_playOver.width())/2,10)+'px');
					bannerscollection_kenburns_playOver.css('top',parseInt((options.height-bannerscollection_kenburns_playOver.height())/2,10)+'px');	
		...
		return this.each(function() {
			...
			if (options.enableTouchScreen) {
				...
				bannerscollection_kenburns_contentHolder.draggable({ 
					...
					start: function(event, ui) {
						origLeft=parseInt(jQuery(this).css('left'),10);
						bannerscollection_kenburns_playOver.css('display','none');
					},
					stop: function(event, ui) {
						if (!current_obj.slideIsRunning) {
							finalLeft=parseInt(jQuery(this).css('left'),10);
							direction=1;
			...
			imgs.each(function() {
              ...
              if (options.skin!="opportune") {
		    		...
		    		jQuery(current_obj.currentImg.attr('data-text-id')).css('left',parseInt((total_images-1)*options.width,10));
